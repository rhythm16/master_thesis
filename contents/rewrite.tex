...
\item \code{tlb\_flush\_ipa}: flushes the input intermediate physical address range of the vmid given.
\end{itemize}

\textbf{KVM ARM Per-CPU Variables in Rust.}
Using KVM ARM Per-CPU variables is a special case for unsafe Rust.
Mainline KVM has its EL2 per CPU variable mechanism (\autoref{fig:percpu});
