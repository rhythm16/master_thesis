...
the techniques we employed to solve them.
...
\code{CHOOSE\_NVHE\_SYM()} that prepends the prefix (\code{\_\_kvm\_nvhe\_})
for referencing NVHE symbols so that we do not have to write
\code{\_\_kvm\_nvhe\_} everytime the code references a NVHE symbol. This makes
...
macro \code{KVM\_NVHE\_ALIAS()}, which creates an additional symbol referring to the
...
\code{arch/arm64/krustvm} is created. Within it contains the \rustcore{} crate
...
To integrate building \code{libkrustvm.a} and linking it with the
...
\code{bindgen}~\cite{bindgen}. And for constants that are used by both Linux
