...
memory region we defined for \rustcore{}'s memory accesses, and
...
these safety rules opens up the potential for null pointer dereferences or
use-after-free memory bugs when raw pointers are accessed. Hence, raw pointer
accesses are prohibited in safe Rust.
...
their implementation. We also deliberately made the amount of unsafe code that
contains raw pointer accesses small ($\sim$50 LOC).
