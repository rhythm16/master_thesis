...
Nevertheless, if we assume the correctness of the safety checks in our memory
region isolation, we can guarantee that \rustcore{} will access the intended
region whenever a raw pointer is accessed.
As for the rest of the unsafe Rust usages, e.g. inline assembly, foreign
function calls, or raw pointer usages within the intended regions, the memory
safety of \rustcore{} still relies on the correctness of their respective
implementations.
...
the compiler cannot check for hardware misconfiguration, such as setting the
wrong permission bits for a NPT entry. Further, software bugs that are not
related to memory such as forgetting to clear a page's contents before changing
the ownership of the page are not prevented by Rust as well.
While Rust
verification tools introduced in \autoref{sec:fw} may offer some assistance
regarding these logical errors, it is clear that they do not match the level of
assurance offered by formal verification.
Autoverification tools are not able to explore all possible program paths and
states comprehensively, as they can only guarantee that individual parts of the
implementation satisfy certain low-level specifications. For instance, a
sorting function may successfully sort an input vector. This limitation makes
verification tools unable to holistically prove the entire codebase
conforms to higher-level security properties. In contrast, formal verification
considers all possible system states and inputs, providing a higher level of
